...
output_folder = '../output_associal_{}v{}_{}-{}/'.format(INPUT_WINDWO_SIZE,HISTORY_WINDOW_SIZE,INPUT_BATCH_SIZE,HISTORY_BATCH_SIZE)
...
def save_dataframe_to_file(df, file_name):
    ...
    df['Test_type'] = df['Test_type'].map(map_type)
    df['Similar_Type'] = df['Similar_Type'].map(map_type)
...
def thread_process(batch_input_files, batch_history_files):
    ...
    # save output
    file_name = datetime.now().strftime("%Y%m%d_%H%M%S") + '.' + result_extension# xls,csv
...
def process_one_by_one(df_input_other, df_history_bidding):
    ...
    for i, row in enumerate(similarity_matrix):
        ...
        # 1.长度规则过滤，低分过滤
        if abs(len(src_title)-len(trg_title)) > 20 or bidding_index == i or float(bidding_score) < candidate_threshold:
            continue
        ...
        if flag == True:
            line.extend(get_history_format_from_df(df_history_bidding,bidding_index))#进一步添加招标公告信息，共8列
            result.append(line)
            continue
        if flag == False:
            continue
        # 3.达到阈值
        if float(bidding_score) >= threshold:
            line.extend(get_history_format_from_df(df_history_bidding,bidding_index))#进一步添加招标公告信息，共8列
            result.append(line)
            continue
        # 3. 候选判决
    ...
    return pd.DataFrame(result, columns=output_format)


def get_history_format_from_df(df,idx):
    '''
    ['ID', 'Title', 'Content', 'Type']
    按照关联格式，构造关联上的历史数据
    '''
    line = []
    # line.append(df['ID'].tolist()[idx].strip())
    # line.append(df['Title'].tolist()[idx].strip())
    # line.append(df['Content'].tolist()[idx].strip())
    # line.append(df['Type'].tolist()[idx].strip())
    line = []
    # line.append(str(df.iloc[idx]['ID']))# id
    # line.append(str(df.iloc[idx]['Title']))  # title
    # line.append(str(df.iloc[idx]['Content'])) #content
    # line.append(str(df.iloc[idx]['Type'])) #type
