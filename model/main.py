...
HISTORY_BATCH_SIZE = 3  # 取决于电脑性能，本机8G内存 <=3
...
data_folder = '../22219v806645_speed_test_raw_data/'
input_folder = data_folder + 'input'
history_folder = data_folder + 'history'
output_folder = '../output_22219v806645_speed_test_raw_data_{}v{}_{}-{}/'.format(INPUT_WINDWO_SIZE,HISTORY_WINDOW_SIZE,INPUT_BATCH_SIZE,HISTORY_BATCH_SIZE)
...
def process_one_by_one(df_input_other, df_history_bidding):
    ...
    for i, row in enumerate(similarity_matrix):
        ...
        if flag == True:
            line.append(str(df_history_bidding.iloc[bidding_index]['ID']))
            line.append(
                str(df_history_bidding.iloc[bidding_index]['Title']))  # 额外加上title，便于观测
            result.append(line)
            continue
        if flag == False:
            continue
